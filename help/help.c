...
static int help_doclist_parse(struct Mailbox *m)
{
  ...
  mutt_str_strfcpy(m->realpath, mutt_str_strdup(C_HelpDocDir), sizeof(m->realpath));

  /* check (none strict) what the user wants to see */
  const char *request = help_path_transpose(m->path, false);
  m->emails[0]->read = false;
  if (request)
  {
    mutt_str_strfcpy(m->path, help_path_transpose(request, false), sizeof(m->path)); /* just sanitise */
